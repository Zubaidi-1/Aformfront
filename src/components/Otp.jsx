...
export default function Otp() {
  ...
  const verifyOtp = async (e) => {
    ...
    if (!email) {
      return setErrorMsg("No email found. Please restart the sign-up flow.");
    }
    ...
    try {
      ...
      const result = await fetch(`${API_BASE}/users/verifyotp`, {
        ...
        body: JSON.stringify({ email, type, otp: otp.trim() }),
      });
    ...
    <div className="min-h-screen flex justify-center items-center bg-gradient-to-r from-indigo-500 to-blue-500">
      ...
      >
        ...
        <button
          type="submit"
          disabled={submitting}
          className="bg-[#192a51] w-full py-3 rounded-lg font-semibold text-white transition transform hover:scale-[1.02] hover:shadow-md disabled:opacity-50 text-center"
        >
