  ...
  const fetchColumns = async () => {
    ...
    try {
      if (isUsersTable) {
        ...
        // include any columns present in rows but missing in meta
        if (rows[0]) {
        ...
        // push notes & flagged to the end for nicer UX
  ...
  const saveCell = async (rowId, col, value, rowRef) => {
    ...
    try {
      if (isUsersTable && col.toLowerCase() === "role") {
        ...
        // normalize flagged to 0/1 for backend
        let toSend = value;
